...
func (bs *BlockStore) MigrateBlocks(inst MigrationInstruction, succ *bool) error {
	...
	// We loop through the current blockStore's blocks
	for hashString, block := range (bs.BlockMap) {
        
		// We try to calculate the index that this block stays in ring
	    hashBytes, _ := hex.DecodeString(hashString)
	    hashInt := new(big.Int).SetBytes(hashBytes[:])
	    ringSizeInt := big.NewInt(int64(bs.RingSize))
	    indexInt := new(big.Int).Mod(hashInt, ringSizeInt)
	    index := int(indexInt.Int64())

		// If this block is in the range, we try to put this block into destination nodes.
		if inst.LowerIndex < inst.UpperIndex {
		    if index >= inst.LowerIndex && index <= inst.UpperIndex {
			    e = conn.Call("BlockStore.PutBlock", block, succ)
		    }
	    } else {
			if index >= inst.LowerIndex || index <= inst.UpperIndex {
